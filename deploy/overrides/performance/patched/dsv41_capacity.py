...
def record_capacity(config, cache, reported_tokens, reported_concurrency):
    ...
    _write('capacity', {**_parallel(config), 'max_model_len': length,
...
def record_dspark(speculator):
    _write('dspark', {**_parallel(speculator.vllm_config), 'speculator': type(speculator).__name__,
...
def record_graph_capture(manager, description):
    if not os.environ.get('DSV41_OBSERVATION_DIR'):
        return
    _write('graph-capture', {**_parallel(manager.vllm_config), 'manager': type(manager).__name__, 'description': description,
...
def record_graph_replay(manager, desc):
    ...
    _write('graph-replay', {**_parallel(manager.vllm_config), 'manager': type(manager).__name__, 'manager_id': id(manager),
