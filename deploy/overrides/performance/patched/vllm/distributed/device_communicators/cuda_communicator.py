...
class CudaCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        self.dp_ag_rs_comm: CustomAllreduce | None = None
        if (unique_name.split(":")[0] == "ep" and self.world_size == 8
                and os.environ.get("VLLM_SM80_EP8_CUSTOM_AG_RS") == "1"
                and current_platform.is_cuda()
                and torch.cuda.get_device_capability(self.device) == (8, 0)):
            self.dp_ag_rs_comm = CustomAllreduce(group=self.cpu_group, device=self.device)
    ...
    def reduce_scatterv(
        ...
        if (self.dp_ag_rs_comm is not None and input_tensor.dtype == torch.bfloat16
                and (sizes is None or len(set(sizes)) == 1)):
            custom_output = self.dp_ag_rs_comm.custom_reduce_scatter(input_tensor)
            if custom_output is not None:
                return custom_output.movedim(0, dim).contiguous()
    ...
    def destroy(self):
        if self.dp_ag_rs_comm is not None:
            self.dp_ag_rs_comm.close()
            self.dp_ag_rs_comm = None
    ...
    def all_gatherv(
        ...
        def _all_gather_single(input_: torch.Tensor, sizes: list[int] | None = None):
            if (sizes is None and self.dp_ag_rs_comm is not None
                    and input_.dtype == torch.bfloat16):
                custom_output = self.dp_ag_rs_comm.custom_all_gather(input_)
                if custom_output is not None:
