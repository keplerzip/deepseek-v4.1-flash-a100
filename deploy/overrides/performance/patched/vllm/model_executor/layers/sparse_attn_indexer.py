...
from vllm.v1.attention.ops.mqa_logits_candidates_triton import (
    candidate_logits_enabled,
    candidate_topk_decode,
    candidate_topk_prefill,
    use_candidate_logits,
    warmup_candidate_logits_triton,
)
...
def sparse_attn_indexer(
    ...
    # Compact scoring is restricted to the reviewed SM80 FP8 TP-only path.
    # Preserve DCP/PCP merging and the candidate-source layer's full scoring.
    compact_supported = (
        current_platform.is_cuda()
        and current_platform.is_device_capability(80)
        and not use_deep_gemm and not use_fp4_cache
        and dcp_world_size == 1 and not use_pcp
    )
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            if chunk.local_total_seq_lens == 0:
                ...
                if current_platform.is_xpu():
                ...
                elif compact_supported and use_candidate_logits(
                    candidate_blocks, candidate_write, candidate_block_size,
                    ...
                    candidate_topk_prefill(
                        ...
                        candidate_block_size, topk_indices, topk_tokens,
                    )
    ...
    if has_decode:
        ...
        compact_decode = (
            compact_supported and decode_metadata.global_seq_lens is None
            and use_candidate_logits(candidate_blocks, candidate_write,
                                     candidate_block_size, decode_metadata.max_seq_len)
        )
        if compact_decode:
            candidate_topk_decode(
                padded_q_quant_cast, kv_cache, shard_weights, seq_lens,
                block_table, candidate_blocks[row_lo:row_hi],
                candidate_block_size, topk_indices, topk_tokens,
            )
            logits = None  # No full-width logits or subsequent dense top-k.
        ...
        if current_platform.is_cuda() and not is_deep_gemm_supported():
            ...
            if num_heads is None:
                ...
                if (current_platform.is_device_capability(80)
                        and self.dcp_world_size == 1 and not self.use_pcp
                        and candidate_blocks is not None and not candidate_write
                        and candidate_block_size == 8 and candidate_logits_enabled()):
                    warmup_candidate_logits_triton(
                        num_heads, head_dim, candidate_block_size,
                        sorted(block_sizes), device,
                    )
