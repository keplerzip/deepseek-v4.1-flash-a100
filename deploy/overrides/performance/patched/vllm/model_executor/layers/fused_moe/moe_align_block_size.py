...
def _moe_align_block_size_deterministic(
    ...
    """
    # Retain the deterministic expert/original-token ordering. Unsupported
    # layouts and non-SM80 devices keep the original reference implementation.
    if (os.environ.get("VLLM_FUSED_STABLE_MOE_ALIGN", "0") == "1"
            and topk_ids.is_cuda and topk_ids.is_contiguous()
            and topk_ids.dtype in (torch.int32, torch.int64)
            and torch.cuda.get_device_capability(topk_ids.device) == (8, 0)
            and 0 < num_experts <= 512 and topk_ids.numel() < 2**31
            and (expert_map is None or (expert_map.is_contiguous() and expert_map.numel() > 0))):
        from .stable_moe_align import stable_moe_align
        stable_moe_align(topk_ids, num_experts, block_size, sorted_ids,
