...
while IFS=, read -r index name uuid memory driver mig; do
  ...
  [[ "$mig" == *Disabled* ]] || fail 'MIG must be disabled for this TP8 deployment'
  driver=${driver// /}
...
((available_kib >= 384*1024*1024)) || fail 'Engram loading budget requires at least 384 GiB currently available host RAM'
