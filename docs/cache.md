...
DP2 的 KV 不跨池共享。服务按 prompt_cache_key 或首条用户消息固定路由，避免同会话在两池之间跳转。X-DSV41-DP-Rank 响应头记录所选池；跨协议请求即使语义相同，也可能因编码不同落到不同池。它不设置缓存 TTL，也不能解释此前 TP8 空闲后零命中的原因。EPLB 只重分布专家，不把两个 KV 池合并。
