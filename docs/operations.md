...
R1.1 / 20260916-perf2 可复用原 R1 镜像，通过累计小包直接备份升级。首次启用新增算子先执行 SM80 数值和图重放检查；依据覆盖层、镜像、驱动和开关缓存通过结果。使用方法、诊断开关及恢复最近备份见 [R1.1 更新](performance-update-20260916.md)。版本身份见 deploy/manifests/update.json，实际安装收据见 deploy/runtime/latest-update.json。
